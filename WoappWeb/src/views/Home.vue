...
const scaricaReportPDF = () => {
  ...
  items.forEach(ex => {
    ...
    const w1Val = isRep ? (estraiRepsValPDF(weekVals[0]) || 0) : parsePeso(weekVals[0], ex, 1);
    let latestVal = 0;
    let latestW = 1;
    for (let w = 6; w >= 1; w--) {
      const wVal = isRep ? (estraiRepsValPDF(ex['ins_week' + w]) || 0) : parsePeso(ex['ins_week' + w], ex, w);
      if (wVal > 0) {
        latestVal = wVal;
        latestW = w;
        break;
  ...
  items.forEach(ex => {
    if (isPercVExercise(ex)) {
      ...
      const w6Reps = estraiRepsValPDF(ex.ins_week6) || 8;
      const w1Weight = parsePeso(ex.ins_week1, ex);
      const w1Reps = estraiRepsValPDF(ex.ins_week1) || 8;
