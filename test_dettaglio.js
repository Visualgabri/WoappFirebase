...
function estraiRepsDaInput(str) {
  ...
  if (matchX) return parseFloat(matchX[2]);
...
function estraiPesoDaInput(str) {
  ...
  if (validWeights.length > 0) return String(Math.max(...validWeights));
  return null;
}
...
const isCorpoLiberoEsercizio = (ex) => {
  ...
  const keywords = [
    ...
    'addominali', 'addome', 'leg raise', 'hyperextension', 'corpo_libero', 'dragon'
  ];
...
const calcolaPropostaCaricoDinamico = (baseWeight, baseReps, baseRIR, currW1Reps, fatica, giorniTrascorsi) => {
  ...
  let dateFactor = 1.0;
  if (giorniTrascorsi > 180) dateFactor = 0.97;
  else if (giorniTrascorsi > 90) dateFactor = 0.98;
  else if (giorniTrascorsi > 30) dateFactor = 0.99;
  ...
  if (pesoPesante >= pesoMedia && pesoMedia > 0) pesoPesante = Math.max(0, pesoMedia - step);
  if (pesoDevastante >= pesoPesante && pesoPesante > 0) pesoDevastante = Math.max(0, pesoPesante - step);
  else if (pesoDevastante >= pesoMedia && pesoMedia > 0) pesoDevastante = Math.max(0, pesoMedia - (2 * step));

  const faticaLower = (fatica || '').toLowerCase().trim();
  if (faticaLower.includes('pesante') || faticaLower === '4') return pesoPesante;
  if (faticaLower.includes('devastante') || faticaLower === '5') return pesoDevastante;
  return pesoMedia;
};
...
const calcolaIncrementoDinamicoMedio = (targetWeek) => {
  ...
  for (let w = 1; w < targetWeek; w++) {
    ...
    if (insVal && String(insVal).trim() !== '' && String(insVal).trim() !== '-') {
      ...
      if (pesoStr) {
        ...
        if (!isNaN(peso) && peso > 0) pesiSettimanali.push({ week: w, peso: peso });
      }
    }
  }
  ...
  for (let i = 0; i < pesiSettimanali.length - 1; i++) {
    ...
    if (p2 > p1) {
      sommaPercentuali += (p2 - p1) / p1;
      conteggio++;
    }
  }
  if (conteggio > 0) return sommaPercentuali / conteggio;
    ...
    storicoEsercizio.value.forEach(prevEx => {
      ...
      for (let w = 1; w <= 6; w++) {
        ...
        if (insVal) {
          ...
          if (pesoStr) {
            const peso = parseFloat(pesoStr);
            if (!isNaN(peso) && peso > 0) pesiEx.push(peso);
          }
        }
      }
  ...
  if (storicoConteggio > 0) return storicoSommaPct / storicoConteggio;
  return 0.025;
};
...
const getBaseWeekInfo = (sett) => {
  ...
  if (sett === 2) {
    ...
    if (baseW === 'W6 Prec.') isPreviousWorkoutW6 = true;
    else baseWNum = 1;
  } else if (sett === 3) {
    baseWNum = 2;
  } else if (sett === 4) {
  ...
  return { baseWNum, isPreviousWorkoutW6, repsBase, repsTarget, pesoBase, baseInsText };
};
...
const proponiProgressioneCaricoRIR = (targetWeek, baseWeekNum, baseInsText) => {
  ...
  if (ghostAutoregolazioneRepsAttiva.value && repsBase !== repsTargetPrescritteBase) {
    if (repsBase > repsTargetPrescritteBase) ratioDinamico = 0.8;
    else ratioDinamico = 0.2;
  }
  ...
  if (ghostAutoregolazioneRepsAttiva.value && repsBase !== repsTargetPrescritteBase) {
    if (repsBase > repsTargetPrescritteBase) {
      ...
      if (proposedWeight > pesoBase) proposedWeight = pesoBase;
    }
  }
  ...
  if (haParentesiQuadre) {
    ...
    if (lastLoggedWeek) {
      ...
      if (prevReps && currReps && currReps < prevReps) {
        return { text: prevIns, peso: prevPeso, label: `W${lastLoggedWeek}`, isOverload: true };
      }
    }

    if (prevIns) return { text: prevIns, peso: prevPeso, label: `W${lastLoggedWeek}`, isRepExercise: isRepEx };
    return null;
  } else {
    if (sett === 2) {
      const baseW = propostaBaseWeek2.value;
      if (baseW === 'W6 Prec.') {
        if (previousWorkout.value) {
          ...
          if (prevW6Text) {
            ...
            if (proposedVal !== null) return { text: String(prevW6Text), peso: proposedVal, label: 'W6 Prec.' };
          }
        }
      }
      ...
      if (w1Ins) {
        ...
        if (proposedVal !== null) return { text: w1Ins, peso: proposedVal, label: 'W1', isRepExercise: false };
      }
    }
    ...
    if (sett === 3) {
      ...
      if (w2Ins) {
        ...
        if (proposedVal !== null) return { text: w2Ins, peso: proposedVal, label: 'W2', isRepExercise: false };
      }
    }
    ...
    if (sett === 4 && isWeek4Scarico.value) {
      ...
      return { text: w2Ins, peso: pesoBase, label: 'W2', isScarico: !isCorpoLiberoRepsSalgono, isRepExercise: isRepEx };
    }
    ...
    if (sett === 6) {
      ...
      if (ghostPRAttackAttivo.value) {
        ...
        const recordVal = ottieniRecordStoricoPerReps(repsTarget);
        if (recordVal && recordVal > 0) {
          if (pesoProposto < recordVal && (pesoProposto >= recordVal * 0.95 || recordVal - 2.5)) {
            pesoProposto = Math.round((recordVal + step) / step) * step;
          }
        }
      }
...
const getGhostLiftSmart = (sett) => {
  ...
  if (smartWeight !== null && smartWeight !== undefined && smartWeight > 0) {
    if (!smartGhost.isScarico) smartGhost.peso = smartWeight;
    if (smartGhost.isPostScarico) smartGhost.pesoProposto = smartWeight;
  }
  return smartGhost;
};
...
const calcolaProposteStoricoPerSettimana = (targetW) => {
        ...
        if (insVal && String(insVal).trim() !== '' && String(insVal).trim() !== '-') {
          ...
          if (weightStr) {
            ...
            if (!isNaN(weight) && weight > 0) {
              ...
              if (inputReps !== null && !isNaN(inputReps) && inputReps > 0) repsNum = inputReps;
              if (repsNum && repsNum > 0) {
                ...
                if (isSameWeek || isPeakWeek || isSameReps) {
                  const rirOriginale = estraiRIRDaPrescrizione(prevEx['des_week' + w]) !== null ? estraiRIRDaPrescrizione(prevEx['des_week' + w]) : getRIRDefault(w);
                  const estimated1RM = weight * (1 + (repsNum + rirOriginale) / 30);
                  const rirTarget = estraiRIRDaPrescrizione(workout.value['des_week' + targetW]) !== null ? estraiRIRDaPrescrizione(workout.value['des_week' + targetW]) : getRIRDefault(targetW);
                  let proposedWeight = estimated1RM / (1 + (targetReps + rirTarget) / 30);
                  if (isCorpoLiberoEsercizio(workout.value) && targetReps > repsNum) {
                    if (proposedWeight > weight) proposedWeight = weight;
                  }
    ...
    if (insVal && String(insVal).trim() !== '' && String(insVal).trim() !== '-') {
      ...
      if (weightStr) {
        ...
        if (!isNaN(weight) && weight > 0) {
          ...
          if (inputReps !== null && !isNaN(inputReps) && inputReps > 0) repsNum = inputReps;
          if (repsNum && repsNum > 0) {
            const rirOriginale = estraiRIRDaPrescrizione(workout.value['des_week' + w]) !== null ? estraiRIRDaPrescrizione(workout.value['des_week' + w]) : getRIRDefault(w);
            const estimated1RM = weight * (1 + (repsNum + rirOriginale) / 30);
            const rirTarget = estraiRIRDaPrescrizione(workout.value['des_week' + targetW]) !== null ? estraiRIRDaPrescrizione(workout.value['des_week' + targetW]) : getRIRDefault(targetW);
            let proposedWeight = estimated1RM / (1 + (targetReps + rirTarget) / 30);
            const isManubri = isManubriEsercizio(workout.value);
  ...
  list.sort((a, b) => {
    if (a.pesoProposto !== b.pesoProposto) return a.pesoProposto - b.pesoProposto;
    if (a.isCurrentMesocycle !== b.isCurrentMesocycle) return b.isCurrentMesocycle ? 1 : -1;
...
async function run() {
  ...
  console.log("--- GETTING ALL B5 HISTORY DOCUMENTS ---");
  const snap = await collectionRef
    .where('ID_cliente', '==', '1')
    .where('des_esercizio', '==', 'Push down con triangolo gomiti larghi')
    .get();
    
  console.log(`Found ${snap.size} documents in B5 history:`);
  snap.forEach(doc => {
    const d = doc.data();
    console.log(`Sheet: ${d.num_scheda} | W1:${d.reps_week1} | W2:${d.reps_week2} | W3:${d.reps_week3} | W4:${d.reps_week4} | W5:${d.reps_week5} | W6:${d.reps_week6}`);
  });
